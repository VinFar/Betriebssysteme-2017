...
int OSMP_Send(const void *buf,int count, int dest){
	int slotid;	
		...
		//~ sem_getvalue(&global.fd_postbox[dest].free_sem,&value_sem);
		
		//~ printf("free sem: %d\n",value_sem);
		
		global.fd_postbox[dest].queue++;
		
		sem_wait(&global.fd_postbox[dest].used_sem);		//Warte auf einen freien Platz
		sem_wait(&global.fd_postbox[FREESLOTS].used_sem);	//Hier auch 
		...
		sem_post(&global.fd_postbox[dest].free_sem);	
...
int OSMP_Recv(void *buf,int count, int *source,int *len){
	...
	sem_wait(&global.fd_postbox[global.rank].free_sem);		
	sem_wait(&global.fd_postbox[FREESLOTS].free_sem);
	...
	*len = global.fd_slot[slotid].size;
	...
	if(*len > count){
		...
		sem_post(&global.fd_postbox[global.rank].used_sem);
		
		return OSMP_ERROR;
	}
	
	memcpy(buf,&global.fd_slot[slotid].buffer,(size_t)count);
...
int OSMP_Finalize(void){
	...
	if(init == OSMP_ERROR){
		...
		sv= global.fd_postbox[global.rank].queue;
		printf("sv: %d\n",sv);
		sv++;
		sv++;
	for(int i=0;i<sv;i++){
	...
	sem_wait(&global.fd_postbox[FREESLOTS].mutex_sem);	
		
	
	
	if(global.fd_postbox[global.rank].first == -1){
		printf("if: %d\n",global.fd_postbox[global.rank].first);
	}else{

	global.fd_slot[global.fd_postbox[FREESLOTS].last].next = global.fd_postbox[global.rank].first;
	global.fd_postbox[FREESLOTS].last = global.fd_postbox[global.rank].last;
	
	}
	
	sem_post(&global.fd_postbox[FREESLOTS].mutex_sem);
...
int shm_config(void **shm_ptr,char *name,int count,int *shm_size){	//fertig
	...
	*shm_size = (off_t)((int)sizeof(shm_header) + count*(int)sizeof(pid_t) + (int)sizeof(shm_postbox)*(count+1) + SLOTS*(int)sizeof(shm_slot));
	...
	if(sem_init(&shmpostbox[FREESLOTS].free_sem, 1, 0)){
		perror("FREE_SEMAPHOR ERROR\n"); return OSMP_ERROR;
	}
	if(sem_init(&shmpostbox[FREESLOTS].used_sem, 1, SLOTS)){
	...
	for(int i=1;i<count+1;i++){			//postbox first und last zunächst auf -1 setzen, da noch keine
			...
			global.fd_postbox[i].queue=0;
			
			if(sem_init(&shmpostbox[i].free_sem, 1, 0)){
			perror("FREE_SEMAPHOR ERROR\n"); return OSMP_ERROR;
			}
			if(sem_init(&shmpostbox[i].used_sem, 1, OSMP_MAX_MESSAGES_PROC)){
